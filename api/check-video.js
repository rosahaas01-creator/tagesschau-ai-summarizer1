...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Verbesserter Regex für MP4 (berücksichtigt auch escaped Slashes in JSON)
        const mp4Regex = /https?:\/\/[^"']+\.mp4/g;
        let mp4Matches = (detailHtml.match(mp4Regex) || []).map(url => url.replace(/\\/g, ''));
        ...
        if (!mp4Res.ok) {
            throw new Error(`Download-Fehler: ${mp4Res.status} ${mp4Res.statusText} für URL: ${directMp4Url}`);
        }
        ...
        await resend.emails.send({
            ...
            subject: `TS 100s Newsletter (${videoId})`,
            html: `<b>Summary:</b> ${jsonResponse.summary}<br><b>Visuals:</b> ${jsonResponse.visuals}`
        });

...
// Wir nutzen explizit die v1 API für maximale Stabilität
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Wir nutzen gemini-1.5-flash (stabilste ID)
        const model = ai.getGenerativeModel({ model: "gemini-1.5-flash" });
        const result = await model.generateContent([
            { fileData: { mimeType: file.mimeType, fileUri: file.uri } },
            { text: "Fasse diese Tagesschau in 100 Sekunden kurz zusammen (Inhalt) und beschreibe kurz die Bilder. Antworte als JSON: { \"summary\": \"...\", \"visuals\": \"...\" }" },
        ]);
        
        const responseText = result.response.text();
        // Manchmal packt Gemini das in Markdown-Blöcke, die wir entfernen müssen
        ...
        // Wir geben hier mehr Details aus, falls das Modell wieder nicht gefunden wird
